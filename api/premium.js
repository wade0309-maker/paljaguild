...
const SYSTEM_PROMPT = [
  "# 역할: 최고위급 명리 전략 컨설턴트",
  "자평명리학과 현대 통계학을 결합한 프리미엄 인생 전략 리포트를 작성하십시오.",
  "명리학 용어는 현대 경영 용어로 치환. 3단 논법: [결론]->[명리적근거]->[현실예시]",
  "",
  "자산 Tier: S급(50억+) A급(15~50억) B급(5~15억) C급. Base 확률 60~70%.",
  "",
  "반드시 아래 JSON을 먼저 출력 후 마크다운 섹션 출력:",
  "",
  "CHART_JSON_START",
  ...
  "",
  "### [1] 명식 검증 (표 형식)",
  "### [2] 운명의 3중 육각형 (표 형식, 6개 항목)",
  "### [4] 본질과 현실 페르소나 (3단 논법)",
  "### [6] 운명의 도플갱어 (역사 인물 1명)",
  "### [7] 재산 시뮬레이션 (표 형식, 3시나리오)",
  "### [9] 부모운 및 관계 리스크 (3단 논법)",
  "### [10] 10년 대운 심층 서사 (3단 논법)",
  "### [11] 단기 자산 흐름도 (표: Jackpot/Crash)",
  "### [12] Executive Summary (실행 과제 3가지)",
  "### [13] 운명 요약 영수증 (> 인용구 형식)"
].join("\n");
...
export default async function handler(req) {
  ...
  if (!input.year || !input.month || !input.day)
    return new Response(JSON.stringify({ error: '생년월일 필수' }), { status: 400, headers });
  ...
  try {
    // 만세력 계산
    const [h, m] = (input.hour_name ? hourNameToTime(input.hour_name) : [input.hour || 12, input.minute || 0]);
    const lon = input.lon || 126.97;
    const baziResult = calculateBazi(
      parseInt(input.year), parseInt(input.month), parseInt(input.day),
      h, m, lon, input.gender === '여' ? 'F' : 'M'
    );

    const p = baziResult.pillars;
    const o = baziResult.ohaeng;
    const payload = [
      "이름: " + (input.name || ""),
      "성별: " + (input.gender || "미상"),
      ...
      "일간: " + p.일주.stem + " (" + p.일주.stemElem + ")",
      "오행분포: 목" + o.목 + " 화" + o.화 + " 토" + o.토 + " 금" + o.금 + " 수" + o.수,
      "격국: " + p.월주.branchSS + "격"
    ].join("\n");
    ...
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: '다음 명식 데이터로 완벽한 프리미엄 리포트를 작성해줘.\n\n' + payload }]
      })
    });
    ...
    let chartData = null;
    const jsonMatch = result.match(/CHART_JSON_START\s*([\s\S]*?)\s*CHART_JSON_END/);
    if (jsonMatch) { try { chartData = JSON.parse(jsonMatch[1].trim()); } catch(e) {} }

    return new Response(JSON.stringify({ result, chartData, baziData: payload }), { headers });
