...
export default async function handler(req) {
  ...
  // ── v3.0 스탯 직접 계산 (JavaScript, 항상 일관된 결과)
  function calcStatsJS(ilganEl, sipseongs, missing, ohaeng) {
    const base = {추진력:50, 사업감각:50, 창의력:50, 통찰력:50, 대인매력:50, 리스크관리:50, 학습력:50, 지속력:50};

    // 일간오행 보정
    const eB = {목:{추진력:15,창의력:10,지속력:-10},화:{대인매력:15,창의력:10,지속력:-10},
      토:{지속력:15,리스크관리:10,추진력:-5},금:{리스크관리:15,학습력:10,대인매력:-10},수:{통찰력:15,학습력:10,지속력:-5}};
    Object.entries(eB[ilganEl]||{}).forEach(([k,v])=>{ base[k]=(base[k]||50)+v; });

    // 십성 보정 (중복 카운트)
    const sB = {비견:{추진력:10},겁재:{추진력:15,대인매력:5,리스크관리:-15},
      식신:{창의력:15,지속력:10,추진력:-5},상관:{창의력:20,통찰력:10,대인매력:8,리스크관리:-10},
      편재:{사업감각:20,대인매력:10,리스크관리:8,지속력:-10},정재:{사업감각:10,리스크관리:15,창의력:-5},
      편관:{통찰력:15,추진력:10,리스크관리:8,대인매력:-10},정관:{리스크관리:15,학습력:10,창의력:-10},
      편인:{통찰력:20,학습력:10,사업감각:-10},정인:{학습력:20,지속력:10,추진력:-5}};

    // 십성 카운트
    const cnt = {};
    sipseongs.forEach(s=>{ cnt[s]=(cnt[s]||0)+1; });

    Object.entries(cnt).forEach(([ss, count]) => {
      const bonus = sB[ss];
      if(!bonus) return;
      const mult = count >= 3 ? 1.5 : count >= 2 ? 1.3 : 1.0;
      Object.entries(bonus).forEach(([k,v])=>{ base[k]=(base[k]||50)+Math.round(v*mult); });
    });

    // 신약패널티 (식상관이 5개 이상)
    const sCount = (cnt['식신']||0)+(cnt['상관']||0)+(cnt['정관']||0)+(cnt['편관']||0);
    if(sCount >= 4) { base.지속력-=15; base.리스크관리-=10; base.대인매력-=8; }

    // 결핍오행 패널티
    const misB = {수:{통찰력:-15},목:{추진력:-15},화:{대인매력:-10},금:{리스크관리:-15},토:{지속력:-15}};
    missing.forEach(el=>{ Object.entries(misB[el]||{}).forEach(([k,v])=>{ base[k]=(base[k]||50)+v; }); });

    // 비견 3개+ 사업감각 보정
    if((cnt['비견']||0) >= 3) base.사업감각 = Math.min(95, (base.사업감각||50)+10);

    // 범위 제한 10-95
    Object.keys(base).forEach(k=>{ base[k]=Math.max(10,Math.min(95,base[k])); });
    return base;
  }

  // ── 클래스 배정 (코드로 직접)
  function assignClassJS(sipseongs, ilganEl) {
    const cnt = {};
    sipseongs.forEach(s=>{ if(s && s!=='비견') cnt[s]=(cnt[s]||0)+1; });
    const topSS = Object.entries(cnt).sort((a,b)=>b[1]-a[1])[0]?.[0] || '비견';

    const mainMap = {겁재:{n:'승부사',e:'⚔️'},편관:{n:'전략가',e:'🗡️'},상관:{n:'혁명가',e:'🔥'},
      편재:{n:'사업가',e:'💰'},정인:{n:'학자',e:'📚'},정관:{n:'관리자',e:'🏛️'},
      비견:{n:'개척자',e:'🌿'},식신:{n:'창작자',e:'🎨'},편인:{n:'예언자',e:'🔮'},정재:{n:'귀족',e:'👑'}};
    const subMap = {
      승부사:{화목:'카리스마 도적',토:'위기 대응형 용병',금수:'본능형 사냥꾼'},
      전략가:{화목:'야전 사령관',토:'암살자형 전략가',금수:'냉혈 참모'},
      혁명가:{화목:'반골형 예술가',토:'선동형 혁명가',금수:'천재 발명가'},
      사업가:{화목:'네트워크형 브로커',토:'기회 포착형 투기사',금수:'투자형 상인왕'},
      학자:{화목:'왕립 현자',토:'실용형 전략가',금수:'은둔형 연구자'},
      관리자:{화목:'왕국 기사단장',토:'조직형 중재자',금수:'제도권 관료'},
      개척자:{화목:'독립형 창업가',토:'외길 장인',금수:'자유 탐험가'},
      창작자:{화목:'감성형 힐러',토:'스토리텔러',금수:'매혹의 연금술사'},
      예언자:{화목:'영적 분석가',토:'패턴 해독사',금수:'직관형 탐정'},
      귀족:{화목:'안정형 수호자',토:'유산 관리형 계승자',금수:'재무 귀족 영주'}
    };

    const main = mainMap[topSS] || mainMap['비견'];
    const grp = ['화','목'].includes(ilganEl)?'화목':ilganEl==='토'?'토':'금수';
    const sub = subMap[main.n]?.[grp] || subMap[main.n]?.['화목'] || '';
    return {main: main.n, emoji: main.e, sub};
  }

  // ── 스탯 태그 매핑
  function getTag(statName, value) {
    const tagMap = {
      추진력:['소극형','균형형','목표형','적극형','돌진형'],
      사업감각:['기본형','관찰형','기회형','포착형','투자형'],
      창의력:['실용형','응용형','창조형','혁신형','천재형'],
      통찰력:['직관형','분석형','패턴형','예지형','선각형'],
      대인매력:['독립형','신중형','신뢰형','친화형','매력형'],
      리스크관리:['도전형','균형형','방어형','분석형','철벽형'],
      학습력:['기본형','응용형','심화형','전문형','통달형'],
      지속력:['자유형','유연형','관계형','안정형','끈기형']
    };
    const tags = tagMap[statName] || ['기본형','보통형','우수형','탁월형','최강형'];
    const idx = value < 40?0:value<55?1:value<70?2:value<85?3:4;
    return tags[idx];
  }

  // ── 실제 계산 실행
  const jsStats = calcStatsJS(ilganElem, sipseongList, missing, ohaeng);
  const jsClass = assignClassJS(sipseongList, ilganElem);

  const statsObj = {};
  Object.entries(jsStats).forEach(([k,v])=>{
    statsObj[k] = {value: v, tag: getTag(k, v)};
  });

  const vals = Object.values(jsStats);
  const totalPower = Math.round(vals.reduce((a,b)=>a+b,0)/vals.length);
  const top2avg = Math.round([...vals].sort((a,b)=>b-a).slice(0,2).reduce((a,b)=>a+b,0)/2);
  const grade = top2avg>=90?'S':top2avg>=80?'A':top2avg>=70?'B':'C';

  // ── Claude에게는 텍스트(조언, 능력, 약점)만 요청
  const textPrompt = "이름:" + name + " 클래스:" + jsClass.main + "(" + jsClass.sub + ") 일간:" + ilganElem +
    " 주요십성:" + sipseongList.join(",") + " 격국:" + geokguk +
    " 오행분포:목" + ohaeng.목 + "화" + ohaeng.화 + "토" + ohaeng.토 + "금" + ohaeng.금 + "수" + ohaeng.수 +
    "\n\n위 캐릭터의 다음 4가지를 JSON으로 출력하라(다른 말 없이 JSON만):" +
    "{hidden_ability:숨겨진능력한줄, weakness:약점한줄, synergy_element:시너지오행한자, synergy_buff:시너지설명한줄, advice:오늘조언20자이내}";
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system:'JSON만 출력. 설명 없음. 한국어.',
        messages:[{role:'user',content:textPrompt}]
      })
    });

    const respText = await resp.text();
    let textData = {hidden_ability:'',weakness:'',synergy_element:'',synergy_buff:'',advice:''};
    if(resp.ok) {
      const rd = JSON.parse(respText);
      const raw = (rd.content?.[0]?.text||'').trim();
      const match = raw.match(/\{[\s\S]*\}/);
      if(match) try { textData = JSON.parse(match[0]); } catch(e) {}
    }
    ...
    const result = {
      ...
      ilgan: p.일주.stem,
      ilgan_element: ilganElem,
      geokguk,
      missing_elements: missing,
      class_main: jsClass.main,
      class_main_emoji: jsClass.emoji,
      class_sub: jsClass.sub,
      element_primary: ilganElem,
      element_secondary: Object.entries(ohaeng).sort((a,b)=>b[1]-a[1])[1]?.[0]||'',
      stats: statsObj,
      total_power: totalPower,
      top2_avg: top2avg,
      potential_grade: grade,
      hidden_ability: textData.hidden_ability||'',
      weakness: textData.weakness||'',
      synergy_element: textData.synergy_element||'',
      synergy_buff: textData.synergy_buff||'',
