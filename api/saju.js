...
export default async function handler(req) {
  ...
  if (!apiKey) return new Response(JSON.stringify({ error: 'Gemini API 키 미설정' }), { status: 500, headers });
...
위 데이터를 기반으로 [0]~[13] 전체 리포트를 출력하십시오.`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Gemini API 오류', detail: errText.slice(0, 300) }), { status: 500, headers });
    }
