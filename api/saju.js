...
export default async function handler(req) {
  ...
  try {
    ...
    let chartData = null;
    // 1. CHART_DATA_START/END 시도
    let jsonStr = null;
    const m1 = raw.match(/CHART_DATA_START\s*([\s\S]*?)\s*CHART_DATA_END/);
    if(m1) jsonStr = m1[1].trim();
    // 2. ```json 코드블록 시도
    if(!jsonStr) { const m2 = raw.match(/```json\s*([\s\S]*?)\s*```/); if(m2) jsonStr = m2[1].trim(); }
    // 3. 첫 { ~ 마지막 } 시도
    if(!jsonStr) { const s=raw.indexOf('{'),e=raw.lastIndexOf('}'); if(s>=0&&e>s) jsonStr=raw.slice(s,e+1); }
    if(jsonStr) { try { chartData = JSON.parse(jsonStr); } catch(e) {} }
